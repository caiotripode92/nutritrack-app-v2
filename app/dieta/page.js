...
export default function DietaPage() {
  ...
  if (loading) return (
    <div className="flex justify-center items-center h-64">
      <div className="text-gray-500">Carregando planos alimentares...</div>
    </div>
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-2xl font-bold text-gray-800">📋 Planos Alimentares</h1>
        <button
          onClick={() => {
            setEditingMeals([])
            setPlanName('')
        ...
        <div className="grid gap-6">
          {dietPlans.map(plan => (
            <div key={plan.id} className="bg-white rounded-lg shadow-md overflow-hidden">
              ...
              {expandedPlans[plan.id] && plan.meals && plan.meals.length > 0 && (
                <div className="p-4 space-y-4">
                  {plan.meals.map((meal, mealIdx) => {
                    const mealTotalKcal = meal.foods.reduce((sum, f) => sum + (f.kcal || 0), 0)
                    const mealTotalProt = meal.foods.reduce((sum, f) => sum + (f.protein || 0), 0)
                    const mealTotalCarbs = meal.foods.reduce((sum, f) => sum + (f.carbs || 0), 0)
                      ...
                      <div key={mealIdx} className="border rounded-lg overflow-hidden">
                        <div className="bg-gray-100 px-4 py-2 font-semibold flex justify-between items-center">
                          ...
                          <span className="text-xs text-gray-500">
                            {Math.round(mealTotalKcal)} kcal | P: {formatMacros(mealTotalProt)}g | C: {formatMacros(mealTotalCarbs)}g
                          </span>
                        </div>
                          ...
                          <table className="w-full text-sm min-w-[500px]">
                            <thead>
                              <tr className="border-b">
                                ...
                                <th className="text-right py-2">Prot (g)</th>
                                <th className="text-right py-2">Carb (g)</th>
                               </tr>
                            </thead>
                            <tbody>
                              {meal.foods.map((food, foodIdx) => (
                                <tr key={foodIdx} className="border-b last:border-0">
                                  ...
                                  <td className="text-right py-2">{formatMacros(food.protein)}</td>
                                  <td className="text-right py-2">{formatMacros(food.carbs)}</td>
                                </tr>
                              ))}
                            </tbody>
                            <tfoot className="bg-gray-50">
                              <tr>
                                ...
                                <td className="text-right py-2 font-semibold">{Math.round(mealTotalKcal)}</td>
                                <td className="text-right py-2 font-semibold">{formatMacros(mealTotalProt)}</td>
                                <td className="text-right py-2 font-semibold">{formatMacros(mealTotalCarbs)}</td>
                              </tr>
                            </tfoot>
                          </table>
                        </div>
                  ...
                  <div className="bg-blue-50 rounded-lg p-4 mt-2">
                    ...
                    <div className="grid grid-cols-3 gap-4 text-center">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{Math.round((plan.totalProtein || 0) * 4)} kcal</p>
                      </div>
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{Math.round((plan.totalCarbs || 0) * 4)} kcal</p>
                      </div>
                    </div>
                    ...
                    {plan.totalKcal < tmb + plan.goalDelta && plan.totalKcal > 0 && (
              ...
              {/* Mensagem se não há refeições */}
              {expandedPlans[plan.id] && (!plan.meals || plan.meals.length === 0) && (
                <div className="p-8 text-center text-gray-500">
                  <p>Este plano não possui refeições cadastradas.</p>
                </div>
              )}
            </div>
          ))}
                    ...
                    {meal.foods.map((food, foodIdx) => (
                      <div key={foodIdx} className="bg-white rounded-lg p-3 mb-2 flex flex-wrap gap-2 items-center">
                        <select
                          ...
                          className="flex-1 min-w-[150px] border rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                        >
                ...
                return (
                  <div className="bg-blue-50 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-3 gap-4 text-center">
                      <div>
                        <p className="text-xs text-gray-600">Calorias</p>
                        <p className="text-xl font-bold text-blue-600">{Math.round(totalKcal)} kcal</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-600">Proteínas</p>
                        <p className="text-xl font-bold text-green-600">{Math.round(totalProtein)}g</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-600">Carboidratos</p>
                        <p className="text-xl font-bold text-orange-600">{Math.round(totalCarbs)}g</p>
                      </div>
                    </div>
                    <div className="mt-3 text-center">
                      <p className="text-sm">
                        Meta: <strong className="text-blue-600">{targetKcal} kcal</strong>
                        {totalKcal > targetKcal && (
                          <span className="text-red-600 ml-2">⚠️ {Math.round(totalKcal - targetKcal)} kcal acima</span>
                        )}
                        {totalKcal < targetKcal && totalKcal > 0 && (
                          <span className="text-orange-600 ml-2">⚠️ {Math.round(targetKcal - totalKcal)} kcal abaixo</span>
                        )}
                      </p>
                    </div>
                  </div>
